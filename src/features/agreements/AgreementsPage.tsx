...
export default function AgreementsPage() {
  ...
  const sheet: Sheet = useMemo(() => {
    ...
    const body = pickRows.map((a) => [
      ...(group ? [group.of(a)] : []),
      ...shown.map((c) => c.text(a)),
    ])
    const totals = [
      ...(group ? [''] : []),
      ...shown.map((c, i) => {
        const t = aggregate(c, pickRows)
        return t !== null ? String(t) : i === 0 ? `${pickRows.length} اتفاقية` : ''
      }),
    ]
    const stamp = new Date().toISOString().slice(0, 10)
    return { file: `abanumay-agreements-${stamp}`, title: 'الاتفاقيات', headers: head, rows: body, totals }
