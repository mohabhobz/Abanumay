...
const ALLOW = [
  ...
  /* ⚠️ **البوّابة صفحة الطلب نفسه (ن-2).** رقم الطلب في ترويستها
     هو معرّف الشاشة اللي إنت فيها · والاستثناء بالمسار لا
     بالمعرّف، لأن الطلب الافتراضي مش مكتوب في الرابط. */
  { kind: 'معرّف نصّي بلا مدخل في صفّه', when: (f) => f.route.startsWith('/entities/portal') },
]
