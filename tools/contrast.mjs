  ...
  const walk=(el)=>{
    for(const n of el.childNodes){
      if(n.nodeType===3 && n.textContent.trim().length>1){
        ...
        /* ⚠️ **النصّ المقصوص لقارئ الشاشة مش نصّ كمان.** `.vis-h`
           صندوقه ١×١ بـ`clip-path:inset(50%)`، والنصّ جوّاه بيفيض
           برّه الصندوق فبياخد `rect` طبيعيًا من النطاق ·
           و`elementFromPoint` بيرجّع الأب المحتوي فيعدّي الحارس
           اللي تحت. النتيجة كانت تمان «مخالفات» لكلمة «تمّت» في
           سُلّم المحطات · نصّ محدش بيشوفه، فوق نقطة خضرا محدش
           بيقراها. والأداة اللي بتبلّغ عن مخالفة وهمية بتتعلّم
           إنها تتتجاهل، فبتضيع المخالفة الحقيقية معاها.
           الاختبار: صندوق العنصر نفسه لا صندوق النصّ. */
